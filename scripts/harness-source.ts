 ...
 *
 * Two sources, one switch — the shape `omdsh-sidechat` uses for the same
 * problem:
 *
 ...
 * @module @omdsh-plugins/omdsh-plughub/scripts/harness-source
 */
...
/** Schemastery ships from the harness's vendor/ on its own upstream version line. */
...
function registrySpecifier(name: string): string {
  if (name === '@deepseek-ai/cordis') return CORDIS_VERSION
  if (name === '@deepseek-ai/schemastery') return SCHEMASTERY_VERSION
